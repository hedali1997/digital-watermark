...
def text2img(text, size_width, mode='1', fontname='SimSun', fontsize=30, size_height=50):
    """文本转图像
    mode='1' 二值图
    mode='RGB' RGB图
    """
    ...
    lines = textwrap(text, size_width, font)
    if mode == '1':
        img = Image.new(mode, (size_width, size_height), 0)
        ...
        img = Image.new(mode, (size_width, size_height), (0, 0, 0))
        dr = ImageDraw.Draw(img)
        y = 0
        for line in lines:
